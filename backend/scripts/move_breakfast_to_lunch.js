/**
 * Script to move all breakfast transactions to lunch
 * This fixes the issue where breakfast was scanned by mistake instead of lunch
 */
...
async function moveBreakfastToLunch() {
    try {
        ...
        // Find lunch resource(s)
        const lunchResources = await Resource.find({
            name: { $regex: /lunch/i }
        });

        if (lunchResources.length === 0) {
            console.log('\n⚠️  No lunch resources found');
            await mongoose.disconnect();
            process.exit(0);
        }

        console.log(`\nFound ${lunchResources.length} lunch resource(s):`);
        lunchResources.forEach(r => {
            console.log(`  - ${r.name} (ID: ${r._id}, Distributed: ${r.distributedQuantity})`);
        });

        // Use the first breakfast and lunch resources (or you can specify which ones)
        const breakfastResource = breakfastResources[0];
        const lunchResource = lunchResources[0];

        console.log(`\n📋 Using:`);
        console.log(`  Breakfast: ${breakfastResource.name} (${breakfastResource._id})`);
        console.log(`  Lunch: ${lunchResource.name} (${lunchResource._id})`);
        ...
        for (const tx of breakfastTransactions) {
            try {
                // Update transaction to use lunch resourceId
                await Transaction.updateOne(
                    { _id: tx._id },
                    { $set: { resourceId: lunchResource._id } }
                );
                movedCount++;
            } catch (error) {
        ...
        // Increment lunch distributedQuantity
        lunchResource.distributedQuantity += breakfastCount;
        await lunchResource.save();
        console.log(`  ✅ Lunch: ${lunchResource.name} - Incremented by ${breakfastCount} (now: ${lunchResource.distributedQuantity})`);
        ...
        const verifyLunch = await Transaction.countDocuments({
        ...
        console.log(`  Lunch transactions: ${verifyLunch}`);
