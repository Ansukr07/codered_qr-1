...
import { Home, ScrollText, Trophy, UserRound, QrCode, ScanLine } from 'lucide-react'
...
export function GameShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="game-world"><div className="pixel-cloud cloud-one"/><div className="pixel-cloud cloud-two"/><header className="game-topbar"><Link href={onboarding ? '/participant/onboarding' : '/participant'} className="game-brand"><span className="brand-cube">CR</span><span>CODERED <b>4.0</b></span></Link><div className="game-status"><span className="status-dot"/> {onboarding ? 'CREATING PLAYER' : 'SYSTEM ONLINE'}</div></header><main className="game-content">{children}</main>{!onboarding && <nav className="game-nav" aria-label="Participant navigation">{nav.map((item) => { const active = pathname === item.href; const Icon = item.icon; return <Link className={active ? 'active' : ''} href={item.href} key={item.href}><Icon/><span>{item.label}</span></Link> })}</nav>}</div>
}
