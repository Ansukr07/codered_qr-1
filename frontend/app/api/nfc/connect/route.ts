...
import { hashNfcToken, hasTrustedOrigin, isValidNfcToken, isValidUuid, normalizeConnectionPair } from '@/lib/nfc'
...
export async function POST(request: NextRequest) {
  ...
  if (!isValidUuid(requestId)) return NextResponse.json({ message: 'Invalid request ID.' }, { status: 400 })

  const { data: previous, error: previousError } = await supabase.from('nfc_events').select('metadata').eq('request_id', requestId).maybeSingle()
  if (previousError) return NextResponse.json({ message: 'Could not check this connection request.' }, { status: 500 })
  if (previous) return NextResponse.json({ message: 'Connection already recorded.', duplicate: true, ...previous.metadata })

  const { data: tag, error: tagError } = await supabase.from('participant_tags').select('id,participant_id,status').eq('public_token_hash', hashNfcToken(token)).maybeSingle()
  if (tagError) return NextResponse.json({ message: 'Could not read this badge.' }, { status: 500 })
  ...
  const { data: existing, error: existingError } = await supabase.from('participant_connections').select('*').eq('participant_low_id', pair.low).eq('participant_high_id', pair.high).maybeSingle()
  if (existingError) return NextResponse.json({ message: 'Could not check this connection.' }, { status: 500 })
  ...
  const { error: eventError } = await supabase.from('nfc_events').insert({ tag_id: tag.id, actor_id: auth.user.userId, actor_role: 'participant', event_type: 'profile_connected', request_id: requestId, metadata: { mutual } })
  if (eventError && eventError.code !== '23505') return NextResponse.json({ message: 'Connection saved, but its NFC audit event could not be recorded.', mutual }, { status: 207 })
