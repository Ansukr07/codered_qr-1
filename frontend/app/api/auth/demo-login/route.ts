...
function configuredDemo(email:string, password:string) {
  const candidates = [
    { email:process.env.DEMO_ALICE_EMAIL, password:process.env.DEMO_ALICE_PASSWORD },
    { email:process.env.DEMO_BOB_EMAIL, password:process.env.DEMO_BOB_PASSWORD },
  ]
  return candidates.find(candidate => candidate.email?.toLowerCase() === email.toLowerCase() && candidate.password === password)
}
...
export async function POST(request: NextRequest) {
  const enabled = process.env.NODE_ENV !== 'production' || process.env.ENABLE_DEMO_ACCOUNTS === 'true'
  if (!enabled) return NextResponse.json({ message: 'Not found' }, { status: 404 })
  ...
  const isLegacyLocal = normalizedEmail === DEMO_EMAIL && password === DEMO_PASSWORD && process.env.NODE_ENV !== 'production'
  if (!isLegacyLocal && !configuredDemo(normalizedEmail, password)) return NextResponse.json({ message: 'Invalid demo credentials.' }, { status: 401 })
  let { data: participant, error } = await supabase.from('participants').select('*').eq('email', normalizedEmail).maybeSingle()
  if (isLegacyLocal && error?.message?.toLowerCase().includes('fetch failed')) return localDemoResponse()
  ...
  response.cookies.set('token', token, { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax', maxAge: 12 * 60 * 60, path: '/' })
  return response
}
