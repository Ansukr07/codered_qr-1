...
export default function DemoLoginPage(){
  ...
  return <main className="demo-login-world"><form className="pixel-panel demo-login-card" onSubmit={submit}><Gamepad2 className="demo-gamepad"/><p className="eyebrow">LOCAL DEVELOPMENT ONLY</p><h1>Demo participant</h1><p>Use this second account in another browser or incognito window. Your current participant session stays signed in.</p><label className="game-field">EMAIL<input type="email" value={email} onChange={e=>setEmail(e.target.value)} required/></label><label className="game-field">PASSWORD<input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="Enter demo password" required/></label>{error&&<p className="game-error">{error}</p>}<button className="pixel-button" disabled={busy}>{busy?'LOADING...':'ENTER DEMO WORLD'} <LogIn/></button><Link href="/login" className="demo-back"><ArrowLeft/> Regular OTP login</Link></form></main>
}
